...
#out_dir=/home/sci/jess/Segmentation_Error/Dalhousie_seg/shape_data/vent_MD_cheat/9samples/shape_models/cleaver_meshes/
...
echo $cleaver_call -I -i $in_dir/$filename $in_dir/$lv_file $in_dir/$rv_file $in_dir/$air_file -j -e -v -f matlab -o $out_dir -B 0.01 -F $F -n $outname

$cleaver_call -I -i $in_dir/$filename $in_dir/$lv_file $in_dir/$rv_file $in_dir/$air_file -j -e -v -f matlab -o $out_dir -B 0.01 -F $F -n $outname
