...
def upsample_heart(Filename, outputfilename,name):
  ...
  r_im1=resample(layerids=[layers[0]],x=dims[0],y=dims[1],z=dims[2]*2,crop='false',kernel='box',replace='false')
  wait_for_layer(r_im1[0])
  de_im = erodefilter(layerid = r_im1[0], radius = 1, replace = True)
  wait_for_layer(de_im)
  de_im = dilatefilter(layerid = de_im, radius = 3, replace = True)
  wait_for_layer(de_im)
  de_im = erodefilter(layerid = de_im, radius = 4, replace = True)
  wait_for_layer(de_im)
  de_im = dilatefilter(layerid = de_im, radius = 2, replace = True)
  wait_for_layer(de_im)
  ...
  de2_im = dilatefilter(layerid = de2_im, radius = 5, replace = True)
  wait_for_layer(de2_im)
  de2_im = erodefilter(layerid = de2_im, radius = 6, replace = True)
  wait_for_layer(de2_im)
  de2_im = dilatefilter(layerid = de2_im, radius = 3, replace = True)
  wait_for_layer(de2_im)

  c_im = crop(layerids=[de2_im],origin='[-64.8343,-299.594,-314]',size='[189.818,189.947,159]',replace='false')
